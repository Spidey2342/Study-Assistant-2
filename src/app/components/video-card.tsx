...
export function VideoCard({
  ...
  return (
    <div className="group rounded-xl md:rounded-2xl overflow-hidden backdrop-blur-xl border border-white/10 hover:border-indigo-500/30 transition-all duration-300">
      ...
      <button
        onClick={() => onPlay?.(url)}
        className="relative aspect-video bg-white/5 overflow-hidden w-full"
      >
        <img
          src={thumbnail}
          alt={title}
      ...
      <div className="p-4 md:p-5" style={{ background: 'var(--glass-gradient)' }}>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            onClick={() => onPlay?.(url)}
            className="px-3 md:px-4 py-1.5 md:py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 transition-colors"
          >
